...
- deploy 可以覆盖运行资产，包括 `release-assets/bin/` 下的完整 KataGo runtime、兼容旧路径的 `katago.exe`、`analysis_config.cfg` 和 model 文件。
...
- 如果 `release-assets/bin/` 存在，递归覆盖复制到 `~/.jcgo/bin/`。
- 如果 `release-assets/katago.exe` 存在，覆盖复制到 `~/.jcgo/bin/katago.exe`，用于兼容旧发布结构。
