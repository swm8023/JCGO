@echo off
powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -Command "& { param([string]$bat, [string]$repo, [Parameter(ValueFromRemainingArguments=$true)][string[]]$deployArgs) $lines = Get-Content -LiteralPath $bat; $idx = [Array]::IndexOf($lines, '# POWERSHELL'); if ($idx -lt 0) { throw 'update-publish.bat payload marker missing' }; $script = ($lines[($idx + 1)..($lines.Count - 1)] -join [Environment]::NewLine); & ([ScriptBlock]::Create($script)) -RepoRoot $repo -Mode update -LogName 'update-publish.bat.log' -DeployArgs $deployArgs }" "%~f0" "%~dp0." %*
exit /b %ERRORLEVEL%
# POWERSHELL
param(
...
try {
  ...
  Write-Host "============================================"
  Write-Host "  JCGO Update Publish"
  Write-Host "============================================"
  Write-Host
  ...
  $stdout = Join-Path $env:TEMP ("jcgo-update-publish-{0}.out" -f $PID)
  $stderr = Join-Path $env:TEMP ("jcgo-update-publish-{0}.err" -f $PID)
  try {
  ...
  Write-Host
  Write-Host "[OK] update publish complete"
  Wait-ForExit
  exit 0
} catch {
