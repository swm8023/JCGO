...
describe('responsive layout CSS', () => {
  ...
  it('fixes the cockpit to the viewport with the board as the dominant work surface', () => {
    ...
    expect(styles).toContain('.try-action-enter {\n  border-color: #047857;\n  background: #059669;')
    expect(styles).toContain('.try-action-exit {\n  border-color: #b91c1c;\n  background: #dc2626;')
  })
  ...
  it('keeps the portrait workspace ordered as toolbar, game info, board, controls, then analysis', () => {
    ...
    expect(styles).toContain('.move-number-stone {\n    width: 30px;\n    height: 30px;')
    expect(styles).toContain('.analysis-rail {\n    grid-row: 4;\n    min-height: 0;\n    max-height: none;\n    grid-template-rows: minmax(108px, 126px) minmax(0, 1fr);')
    expect(styles).toContain('.analysis-overview {\n    grid-template-rows: 18px minmax(82px, 1fr);')
    expect(styles).toContain('.winrate-chart {\n    min-height: 82px;')
    expect(styles).toContain('.sidebar-analysis {\n    grid-column: 2;')
  })
