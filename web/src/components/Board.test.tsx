...
describe('Board', () => {
  it('renders ownership, dead stone marks, candidate colors, and current move quality when enabled', () => {
    ...
    expect(board.querySelector('#ownership-mask-soften feGaussianBlur')).toHaveAttribute('stdDeviation', '9')
    expect(board.querySelector('.ownership-edge-fade-field')).toHaveAttribute('filter', 'url(#ownership-mask-soften)')
    expect(board.querySelector('.ownership-edge-fade.right')).not.toBeInTheDocument()
    expect(board.querySelector('.ownership-soft-layer.black')).toHaveAttribute('filter', 'url(#ownership-soften)')
    expect(board.querySelector('#ownership-soften feGaussianBlur')).toHaveAttribute('stdDeviation', '11')
    ...
    expect(screen.getByLabelText('Current move Q16').tagName).toBe('g')
    expect(screen.getByLabelText('Current move Q16').querySelector('.current-move-tick')).toHaveAttribute('stroke', '#f2c15d')
    expect(screen.getByLabelText('Current move Q16').querySelector('circle')).not.toBeInTheDocument()
